...
Диаграмма последовательности описывает обмен сообщениями между участниками во времени: порядок вызовов, ответов и жизненный цикл объектов. Применяется для детализации сценариев взаимодействия компонентов и API.

## Описание примера

Приведены два сценария. В Mermaid — упрощённый обмен между CRON заявителей, backend заявителя и backend сервиса. В PlantUML — загрузка SPA-страниц /main и /catalog с балансировкой, троттлингом, запросами к backend и CDN.
